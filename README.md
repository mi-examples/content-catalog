...
A Metric Insights custom app (Portal Page) that presents a greeting hero header and a
collapsible hierarchy of **Folders** or **Categories** with their elements. Clicking an
element in the menu opens it in the element viewer, which fills the main pane.

Styling follows the Truist-derived theme in `TruistTheme/` — Truist Purple `#2E1A47`,
Archivo headings, Inter body text.
...
authentication provider) require **Node.js 24+**; this machine has Node 22 and 23 only.
Install Node 24 before upgrading, and run `npx @metricinsights/pp-dev migrate` — the 1.x
config schema is different (`mi: { url, token }` / `app: { id }`).
